...
interface DepotSceneProps {
  ...
  prefs: { quality: string; refreshSec: number; defaultTerminal: number; view2DMode?: "grid" | "structured" };
}
...
export default function DepotScene({
  ...
  const [view2DMode, setView2DMode] = useState<"grid" | "structured">(prefs.view2DMode || "structured");
  ...
  return (
    ...
    >
      ...
      {currentQuality === "2d" ? (
        view2DMode === "structured" ? renderStructured2DMap() : render2DMap()
      ) : (
        <Canvas
          shadows
          frameloop="demand"
                    ...
                    <div className="field" style={{ marginBottom: 0 }}>
                      ...
                      <SearchableSelect
                        ...
                        onChange={(val) => {
                          ...
                          setManovrType(code);
                          // اگر استاتیک (4) یا تعویض کفشک (20) است، مقصد روی همین خط تنظیم شود
                          if (code === 4 || code === 20) {
                            setManovrDestLineId(selectedLine.id);
                            const selTrain = lineTrains.find((t) => t.id === Number(manovrTrainId));
                            if (selTrain) setManovrSlotIdx(selTrain.slotIndex);
